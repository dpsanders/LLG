...
function chooseDirectionNew(direction, mirror_index, L)
	...
	for starting_side in 1:4
		for x_start in 1:L
			...
			while true
	            ...
	            direction = reflection[mirror_type+2, direction]
	            ...
	            if mirror_type != 0
	            	if paths[mirror_index] == 0  # not hit yet
...
function findClosedPaths!(L, mirrors, mirror_types, paths, OC_mirror_list, path_number)
	...
	for mirror_index in OC_mirror_list
		...
		direction = (touching_path > 0) ? down : up
		...
		while true
			...
			mirror_index = chooseDirectionNew(direction, mirror_index, L)
